...
// Replace AnimatedHeroTitle components
code = code.replace(/<motion\.span\s*key={key}\s*variants={letterVariants}\s*/g, `<span key={key} className="inline-block text-[#c084fc] drop-shadow-[0_0_20px_rgba(192,132,252,1)] font-black mx-[0.5px] animate-title-char" style={{ animationDelay: \`\$\{0.15 + parseInt(key) * 0.08\}s\`, `);
